...
con1 <- file("D:/Learning/r/coursera/capstone/final/en_US/en_US.twitter.txt", open = "rb")
twitter <- readLines(con1, skipNul = TRUE, encoding="UTF-8")
close(con1)

con2 <- file("D:/Learning/r/coursera/capstone/final/en_US/en_US.news.txt", open = "rb")
news <- readLines(con2, skipNul = TRUE, encoding="UTF-8")
close(con2)

con3 <- file("D:/Learning/r/coursera/capstone/final/en_US/en_US.blogs.txt", open = "rb")
...
writeLines(SampleAll, "D:/Learning/r/coursera/capstone/final/sampleall/SampleAll.txt")
...
SampleAll <- VCorpus(DirSource("D:/Learning/r/coursera/capstone/final/sampleall", encoding = "UTF-8"))

# tokenizing sampled text 
SampleAll <- cleansing(SampleAll)
remove(SampleBlog)
remove(SampleNews)
remove(SampleTwitter)
remove(twitter)
...
tdm_1gram_df <- ngram_sorted_df(tdm_Ngram(SampleAll, 1))
tdm_2gram_df <- ngram_sorted_df(tdm_Ngram(SampleAll, 2))
tdm_3gram_df <- ngram_sorted_df(tdm_Ngram(SampleAll, 3))
tdm_4gram_df <- ngram_sorted_df(tdm_Ngram(SampleAll, 4))
...
write.csv(quadgram[quadgram$freq > 1,],"D:/Learning/r/coursera/capstone/final/quadgram.csv",row.names=F)
quadgram <- read.csv("D:/Learning/r/coursera/capstone/final/quadgram.csv",stringsAsFactors = F)
saveRDS(quadgram,"D:/Learning/r/coursera/capstone/final/quadgram.RData")
...
write.csv(trigram[trigram$freq > 1,],"D:/Learning/r/coursera/capstone/final/trigram.csv",row.names=F)
trigram <- read.csv("D:/Learning/r/coursera/capstone/final/trigram.csv",stringsAsFactors = F)
saveRDS(trigram,"D:/Learning/r/coursera/capstone/final/trigram.RData")
...
write.csv(bigram[bigram$freq > 1,],"D:/Learning/r/coursera/capstone/final/bigram.csv",row.names=F)
bigram <- read.csv("D:/Learning/r/coursera/capstone/final/bigram.csv",stringsAsFactors = F)
saveRDS(bigram,"D:/Learning/r/coursera/capstone/final/bigram.RData")